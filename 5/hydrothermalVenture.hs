...
splitInput input = map (toMoreLine . map toPoint . splitOn " -> ") (lines input)
    where
...
toMoreLine :: [Point] -> [Point]
toMoreLine ((x1,y1):(x2,y2):_) | x1 == x2 = zip (repeat x1) ys
                               | y1 == y2 = zip xs (repeat y1)
                               | otherwise = zip xs ys
                                  where
                                   xs = if x1 < x2 then [x1..x2] else reverse [x2..x1]
                                   ys = if y1 < y2 then [y1..y2] else reverse [y2..y1]
toMoreLine _ = error "something went wrong x_X"
